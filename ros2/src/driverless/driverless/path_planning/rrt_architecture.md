...
- **`rrt_star.py`**: Contains the main `KinematicRRTStar` class.
    ...
    - **Steering (Kinematic):** Integrates bicycle model kinematics under maximum steering angle constraints using a Pure Pursuit-based steering controller.
    - **Cost/Rewiring (RRT*):** Calculates trajectory costs including curvature penalties (to favor straighter paths). Connects new nodes to the best available parents and performs rewiring using iterative BFS to update cost values recursively, avoiding Python's recursion limit.
...
- **`rrt_node.py`**: The `rclpy` node.
    ...
    - **Execution Logic (10 Hz):**
        - Tracks seen cones. Only triggers a replan when `NEW_CONE_THRESHOLD` (2) new cones are detected on both sides, or if no path exists.
        - Stitches path: Starts the RRT* search from the 4th node from the end of the previous path for smooth continuity.
